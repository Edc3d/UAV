...
if __name__ == '__main__':
    try:
        ...
        while not rospy.is_shutdown():
            ...
            case = 1 ## declare rotor-mode as start

            if abs(goal_x - x) > 5 or abs(goal_y - y) > 5: # far away FW MODE
                case = 2
                #rospy.loginfo("CASE : FW")
            if abs(goal_x - x) < 5 or abs(goal_y - y) < 5: # CLOSE RM MODE
                case = 1
                #rospy.loginfo("CASE : RM")
            if case == 1:
                #RM
                h = Header()
                state = 3
